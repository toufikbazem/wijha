  ...
  return (
    <div className="flex justify-center items-center gap-3 p-4 w-full">
      <button
        ...
        disabled={page === 1}
        className="min-w-9 h-9 px-2 rounded-lg border border-gray-200 bg-white text-gray-500
        hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-sm flex items-center justify-center"
      >
      ...
      {items.map((item, index) =>
        item === "..." ? (
          <span
            key={`dots-${index}`}
            className="min-w-9 h-9 flex items-center justify-center text-gray-400 text-sm tracking-widest select-none"
          >
            ···
          </span>
        ) : (
