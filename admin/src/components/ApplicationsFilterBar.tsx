...
type Props = {
  form: UseFormReturn<ApplicationsFilters>;
  onApply: (values: ApplicationsFilters) => void;
  onReset: () => void;
  loading: boolean;
};
...
export default function ApplicationsFilterBar({
  ...
  const applyAppliedIn = (value: string) => {
    const next = value === "any" ? "" : value;
    form.setValue("applied_in", next, { shouldDirty: true, shouldTouch: true });
    onApply({ ...form.getValues(), applied_in: next });
  };
    ...
    <form onSubmit={form.handleSubmit(onApply)}>
      <div className="bg-white p-3 sm:p-4 rounded-lg shadow-sm border border-gray-200">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          {/* Right-side controls: applied_in + sort */}
