...
import {
  ...
  FILTER_KEYS,
  filtersFromParams,
  employersFilterSchema,
  type EmployersFilters,
} from "@/lib/employersFilters";
...
export default function EmployersPage() {
  ...
  const form = useForm<EmployersFilters>({
    resolver: zodResolver(employersFilterSchema),
  ...
  const onApply = (values: EmployersFilters) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${import.meta.env.VITE_API_URL}/admin/employers?${params}`, {
      credentials: "include",
    })
      .then((r) => r.json())
      .then((data) => {
          ...
          <div className="w-full overflow-x-auto">
            <table className="w-full table-fixed">
              ...
              <tbody className="divide-y divide-gray-200">
                {employers.map((e) => (
                  <EmployerItem key={e.id} employer={e} />
                ))}
              </tbody>
            </table>
            {totalPages > 1 && (
